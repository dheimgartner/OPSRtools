...
@
%
shere \code{printLevel = 0} is passed to \fct{opsr} and avoids printing working information during maximum likelihood estimation.
...
%
<<cache-kfold, echo=FALSE>>=
cache_kfold <- readRDS("./cache_kfold.rds")
kfold <- cache_kfold$kfold
kfold_10 <- cache_kfold$kfold_10
kfold_aic <- cache_kfold$kfold_aic
@
%
...
plot.it <- function() {
  ...
  for (i in c("ll_mean", "ll_p_mean", "r2")) {
    kfplot(list(kfold_10, kfold_aic, kfold), i = i, col = colvec)
    legend("bottomleft", legend = c("fit_10", "fit_aic", "fit"), bty = "n",
    fill = colvec)
  }
}
plot.it()
...
\begin{figure}[t!]
\centering
<<plot-kfplot, echo=FALSE, fig=TRUE, height=6.5, width=7>>=
<<kfplot>>
plot.it()
@
...
pairs(ate, labels.diag = c("NTWing", "NUTWing", "UTWing"), col = colvec,
  ...
  font.labels = 2,lwd.dens = 2.5, lty.diag = 4, xlim = c(0, 850),
  ylim = c(0, 850))
@
