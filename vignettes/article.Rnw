...
plot.it <- function(...) {
  ...
  ylab <- c("OOS log-likelihood full model", "OOS log-likelihood selection model", expression(paste("OOS ", R^2, "")))
...
\begin{leftbar}
Explain the code output, e.g., treatment effects.
\end{leftbar}
...
## TODO
## add text to cross (topright and bottomleft)
...
plot.it <- function() {
  ...
  for (i in 1:3) {
    for (j in 1:3) {
      ...
      legend("bottomleft", legend = c("fit_10", "fit_aic"), pch = 19,
