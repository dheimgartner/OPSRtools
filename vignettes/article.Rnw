...
fit_aic_nocor <- opsr(fit_aic$formula, timeuse, weights = timeuse$weights,
...
In conclusion, it is not actually the direct influence of the error correlation via the Heckman correction term but the indirect influence via the beta coefficients that leverages the predictions and thus treatment effect computations. And if we only have few explanatory variables then the error correlation might have a bigger influence on these estimates.
